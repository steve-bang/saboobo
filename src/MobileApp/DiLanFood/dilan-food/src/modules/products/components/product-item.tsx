    ...
    <>
      ...
      >
        <div className="aspect-w-16 aspect-h-12">
          <img src={item.urlImage} alt={item.name} className="w-full h-full object-center object-cover" />
        </div>
    ...
    <div className="max-h-[80vh] overflow-auto primary-scrollbar">
      <div className="h-full">
        <div className="p-3 pb-4">
          <div className="aspect-w-16 aspect-h-12">
            <img src={item.urlImage} alt={item.name} className="w-full h-full object-center object-cover rounded-lg" />
          </div>
        </div>
...
function OptionsSheetContent({
  ...
  item: IProductType
  onClose: () => void
  onAdd: (toppings?: IToppingType[]) => void
}) {
  const [selected, setSelected] = useState(new Map<IToppingType['id'], IToppingType>())
